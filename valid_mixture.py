...
for batch in val_loader_2:
    # inputs without autodiff (no_grad)
    input_ids = batch.input_ids.to(device)
    attention_mask = batch.attention_mask.to(device)
    graph_batch = batch.to(device)
    ...
    # Store embeddings
    graph_embeddings_2.extend(x_graph.detach().cpu().numpy())
    text_embeddings_2.extend(x_text.detach().cpu().numpy())

# # Create a mapping from CID to index
# cid_to_index_2 = {cid: idx for idx, cid in enumerate(val_dataset_2.cids)}
...
print("Validation LRAP Score (Mixture model):", val_lrap_mixture_models)